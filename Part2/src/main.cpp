...
int main(int argc, char** argv)
{
	...
	cudaGLRegisterBufferObject( planetNBO );
...
void runCuda()
{
	...
	float *dptrvertn=NULL;
    float *dptrvert=NULL;
    cudaGLMapBufferObject((void**)&dptrvert, planetVBO);
	cudaGLMapBufferObject((void**)&dptrvertn, planetNBO); 

    // execute the kernel
    cudaNBodyUpdateWrapper(DT);
#if VISUALIZE == 1
    cudaUpdateVBO(dptrvert, dptrvertn, field_width, field_height);
#endif
    // unmap buffer object
	cudaGLUnmapBufferObject(planetNBO);
...
void display()
{
    ...
    char title[100];
    sprintf( title, "565 NBody Sim : Flocking Sim [%0.2f fps]", fps );
...
void init(int argc, char* argv[])
{
    ...
    glutCreateWindow("565 NBody sim : Flocking Sim");
...
void initVAO(void)
{
	...
	GLfloat *velocities = new GLfloat[3*(N_FOR_VIS+1)];
    GLuint *indices    = new GLuint[6*num_faces];
    GLuint *bindices   = new GLuint[N_FOR_VIS+1];

    glm::vec4 ul(-1.0,-1.0,2.0,1.0);
    glm::vec4 lr(1.0,1.0,-1.0,0.0);
...
void initShaders(GLuint * program)
{
	...
	// Turn off shaded height field grid
