...
int main(int argc, char** argv)
{
    ...
    cudaGLRegisterBufferObject( planetAngleVBO );
...
void runCuda()
{
    ...
    float *dptrvert=NULL;
	float *dptrpang=NULL;
	cudaGLMapBufferObject((void**)&dptrpang, planetAngleVBO);
    ...
    cudaUpdateVBO(dptrvert, dptrpang, field_width, field_height);
#endif
    // unmap buffer object
    cudaGLUnmapBufferObject(planetVBO);
    //cudaGLUnmapBufferObject(pbo);
	cudaGLUnmapBufferObject(planetAngleVBO);
}
...
void display()
{
    ...
    glEnableVertexAttribArray(positionLocation);
	glEnableVertexAttribArray(angleLocation);
	...
	glBindBuffer(GL_ARRAY_BUFFER, planetAngleVBO);
    glVertexAttribPointer((GLuint)angleLocation, 1, GL_FLOAT, GL_FALSE, 0, 0);
    ...
    glDisableVertexAttribArray(positionLocation);
	glDisableVertexAttribArray(angleLocation);
...
void initVAO(void)
{
	...
	GLfloat *bodyAngles = new GLfloat[N_FOR_VIS+1];
	...
	glBindBuffer(GL_ARRAY_BUFFER, planetAngleVBO);
    glBufferData(GL_ARRAY_BUFFER, (N_FOR_VIS+1)*sizeof(GLfloat), bodyAngles, GL_DYNAMIC_DRAW);
