...
void display()
{
    ...
    char title[100];
    sprintf( title, "565 Flocking sim [%0.2f fps]", fps );
...
void initVAO(void)
{
	...
	GLfloat *velocities = new GLfloat[4*(N_FOR_VIS)];
	...
	glBindBuffer(GL_ARRAY_BUFFER, planetVelBO);
	glBufferData(GL_ARRAY_BUFFER, 4*(N_FOR_VIS)*sizeof(GLuint), velocities, GL_STATIC_DRAW);
